...
const telegram= window.Telegram.WebApp
  ...
  const App = () => {
    ...
    if(existItem){
      const newData = cartItems.map(c=>c.id==item.id?{...existItem , quantity: existItem.
    quantity+1}:c
    );
  ...
  const onCheckout=()=>{
    ...
    return (
      <>
      <h1 className='heading'>Sardor Eshmurodov</h1>
      <Cart cartItems={cartItems} onCheckout={onCheckout}/>
      <div className='cards__container'>
        {courses.map(course => (
         <Card key={course.id} course={course} onAddItem={onAddItem}
